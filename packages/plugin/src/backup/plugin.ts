  ...
  return (context: PiniaPluginContext) => {
    ...
    Object.keys(actions).forEach(actionName => {
      ...
      if (actionPrefix) {
        ...
        const stateSerializer = getStateSerializer(cnPersist, stateKey);
        ...
        if (!stringValue) {
          ...
          switch (persistType) {
            case 'STRING':
              restoreString(stringValue, stateKey, context, cnPersist);
              break;
            case 'HASH_RESET':
            case 'HASH':
              restoreHash(stringValue, persistKey, stateKey, context, cnPersist);
              break;
          }
    ...
    if (Object.keys(actionListenerPersisterRegistry).length > 0) {
       ...
       *
       * 之所以不用 $subscribe 来监听变化，是因为如果 state 为 Record 类型
       * Record 对象上如果一开始不存在某些 key，那么对这些 key 的 Record.set(...) 操作是不会触发 mutation 的
       */
